...
import { organizationId as orgId } from '@accessforge/domain';
...
const createEnvironmentBody = (body: unknown) => {
  ...
  return { name, baseUrl, type: type as 'LOCAL' | 'PREVIEW' | 'DEVELOPMENT' | 'STAGING' | 'PRODUCTION' };
};
...
export function projectRoutes(opts: ProjectRoutesDeps): (app: FastifyInstance) => void {
  const { repos } = opts;
  return (app) => {
    app.get('/projects', { preHandler: [requirePermission('project.read')] }, async (request, reply) => {
      const orgId = request.cloud.tenant!.organizationId;
      const projects = await repos.projects.listForOrganization(orgId);
      return reply.code(200).send({ projects });
    });

    app.post('/projects', { preHandler: [requirePermission('project.write')] }, async (request, reply) => {
    ...
    app.get('/projects/:projectId', { preHandler: [requirePermission('project.read')] }, async (request, reply) => {
      const orgId = request.cloud.tenant!.organizationId;
      const params = request.params as Record<string, unknown>;
      const projectId = params['projectId'];
      if (typeof projectId !== 'string' || !projectId) throw new BadRequest('invalid projectId');
      const project = await repos.projects.findById(orgId, orgId(projectId) as unknown as ProjectId);
    ...
    app.get(
      ...
      async (request, reply) => {
        ...
        const projectId = params['projectId'];
        if (typeof projectId !== 'string' || !projectId) throw new BadRequest('invalid projectId');
        const environments = await repos.environments.listForProject(
          orgId,
          orgId(projectId) as unknown as ProjectId,
        );
    ...
    app.post(
      ...
      async (request, reply) => {
        ...
        const environment = await repos.environments.insert({
          organizationId: orgId,
          projectId: orgId(projectId) as unknown as ProjectId,
          name: body.name,
          baseUrl: body.baseUrl,
          type: body.type,
        });
